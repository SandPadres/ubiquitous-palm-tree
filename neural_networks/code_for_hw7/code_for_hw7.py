...
class Linear(Module):
    ...
    def forward(self, A): # A is A_l-1
        self.A = A   # (m x b)  Where b is batch size
        return self.W.T@A + self.W0  # (n x m) @ (m x b) = (n x b)

    def backward(self, dLdZ):  # dLdZ is (n x b), uses stored self.A
        # print(self.A.shape)
        # print(dLdZ.shape)
        self.dLdW  = self.A@dLdZ.T  # Your code (m x n)
        # print(self.dLdW.shape)
        self.dLdW0 = dLdZ  # Your code (n x b)
    ...
    def sgd_step(self, lrate):  # Gradient descent step
        ...
        # print(self.W0)
        # print(self.dLdW0)
        # index = np.random.randint(0, self.dLdW0.shape[1])
        summed = np.sum(self.dLdW0, axis = 1, keepdims=True)
        # print(self.W0 - lrate * summed)
        # print(self.W0 - lrate * self.dLdW0[:, :index + 1])
        self.W0 = self.W0 - lrate * summed # Your code
...
class Tanh(Module):  # Layer activation
    ...
    def backward(self, dLdA):  # Uses stored self.A
        # derivative of dtanH(Z)/dZ = 1-a**2
...
class ReLU(Module):  # Layer activation
    ...
    def backward(self, dLdA):  # uses stored self.A
        der = self.A.copy()
        der[der>0] = 1
        return dLdA * der  # Your code: return dLdZ (?, b)


class SoftMax(Module):  # Output activation
    def forward(self, Z): # Z = n x b
        return np.exp(Z)/sum(np.exp(Z))  # Your code: (n, b)
    ...
    def class_fun(self, Ypred):  # Return class indices
        # print(Ypred)
        # print(np.argmax(Ypred, axis=0, keepdims=True))
        # print(np.argmax(Ypred, axis=0).reshape(1,Ypred.shape[1]))
        return np.argmax(Ypred, axis=0).reshape(1,Ypred.shape[1])  # Your code: (1, b)
...
class NLL(Module):  # Loss
    def forward(self, Ypred, Y):
        ...
        # print(Y)
        # print(Ypred)
        return -1 * np.sum(Y * np.log(Ypred))  # Your code: return loss (scalar)
...
class Sequential:
    ...
    def sgd(self, X, Y, iters=100, lrate=0.005):  # Train
        ...
        # print(X.shape)
        # print(Y.shape)
        # indx = np.random.randint(0,Y.shape[1])
        # print(indx)
        # a = X[:,indx:indx + 1]
        # b = Y[:,indx: indx + 1]
        # print(a)
