...
def sgd(X, y, J, dJ, w0, step_size_fn, max_iter):
    ...
    for t in range(max_iter):
        ...
        fs.append(J(x, y_i, w))
        ws.append(w)
        w_new = w - step_size_fn(t) * dJ(x, y_i,w)
    return (w,fs,ws)
...
auto_data_all = load_auto_data(f"{REGRESSION_FILE_PATH}/code_and_data_for_hw05/auto-mpg-regression.tsv")
...
transformation = make_polynomial_feature_fun(3)
print(xval_learning_alg(transformation(auto_data[0]), auto_values, 60, 10) * sigma)
